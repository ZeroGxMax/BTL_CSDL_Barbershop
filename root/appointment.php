	...
	<div class="container">
		...
		<?php
            ...
            if(isset($_POST['submit_book_appointment_form']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            {
                ...
                $customer_phone_number = test_input($_POST['customer_phone_number']);
                ...
                try
                {
					// Check If the customer's email already exist in our database
					$stmtCheckCustomer = $con->prepare("SELECT * FROM customers WHERE customer_email = ?");
                    $stmtCheckCustomer->execute(array($customer_email));
					$customer_result = $stmtCheckCustomer->fetch();
					$customer_count = $stmtCheckCustomer->rowCount();
					...
					{
						$stmtgetCurrentCustomerID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'barbershop' AND TABLE_NAME = 'customers'");
            
						$stmtgetCurrentCustomerID->execute();
						$customer_id = $stmtgetCurrentCustomerID->fetch();

						$stmtCustomer = $con->prepare("insert into customers(first_name,last_name,phone_number,customer_email) 
									values(?,?,?,?)");
						$stmtCustomer->execute(array($customer_first_name,$customer_last_name,$customer_phone_number,$customer_email));
					}
                    ...
                    $stmt_appointment = $con->prepare("insert into appointments(date_created, customer_id, employee_id, start_time, end_time_expected ) values(?, ?, ?, ?, ?)");
                    $stmt_appointment->execute(array(Date("Y-m-d H:i"),$customer_id[0],$selected_employee,$start_time,$end_time));
		...
		<form method="post" id="appointment_form" action="appointment.php">
			...
			<div class="customer_details_div tab_reservation" id="customer_tab">
                ...
                <div>
                    <div class="form-group colum-row row">
                        ...
                        <div class="col-sm-6">
                            <input type="email" name="customer_email" id="customer_email" class="form-control" placeholder="E-mail">
							<span class = "invalid-feedback">Invalid E-mail</span>
                        </div>
                        <div class="col-sm-6">
                            <input type="text"  name="customer_phone_number" id="customer_phone_number" class="form-control" placeholder="Phone number">
